...
export function ModelsCatalogue({ vehicles, categories }: { vehicles: Vehicle[]; categories: string[] }) {
  const [selected, setSelected] = useState(allModels);
  const visibleVehicles = useMemo(() => selected === allModels ? vehicles : vehicles.filter((vehicle) => vehicle.category === selected), [selected, vehicles]);
  ...
  return <>
    ...
    <p aria-live="polite" className="sr-only">Showing {visibleVehicles.length} {visibleVehicles.length === 1 ? "model" : "models"} in {selected}</p>
    <div className="mx-auto w-full max-w-[1440px] px-5 pt-12 sm:px-8 lg:px-12 xl:px-16">{visibleVehicles.length ? <div className="flex min-h-[560px] flex-wrap justify-center gap-5">{visibleVehicles.map((vehicle) => <div key={vehicle.slug} className="flex w-full sm:w-[calc(50%-0.625rem)] xl:w-[calc(33.333%-0.875rem)]"><ListingVehicleCard vehicle={vehicle} index={vehicles.findIndex((item) => item.slug === vehicle.slug)} /></div>)}</div> : <div className="flex min-h-[360px] items-center border border-white/10 bg-white/[.02] p-8"><div><h2 className="text-2xl font-semibold uppercase">More journeys are coming</h2><p className="mt-4 max-w-lg text-sm leading-7 text-white/55">No model records are currently available in this category. Explore another category to continue.</p></div></div>}</div>
  </>;
}
