...
export function ListingVehicleCard({ vehicle, index }: { vehicle: Vehicle; index: number }) {
  const theme = categoryThemes[vehicle.category] ?? "bg-[linear-gradient(145deg,#24272a,#101214)]";
  const hasStudio = vehicle.pageReady && vehicle.colors.some((color) => color.available && color.image);
  ...
  return <article className="group flex w-full min-w-0 flex-col overflow-hidden border border-white/10 bg-[#0d1012] transition-colors duration-300 hover:border-white/28 focus-within:border-white/35">
    <div className="flex items-center justify-between px-6 pb-2 pt-6"><div><p className="text-3xl font-semibold tracking-[-.04em] uppercase">{vehicle.name}</p><p className="mt-1 text-[10px] tracking-[.14em] text-white/45 uppercase">{vehicle.category}</p></div><span aria-hidden="true" className="text-[10px] tracking-[.16em] text-white/25">0{index + 1}</span></div>
    ...
    {hasStudio ? <div className="px-2"><CatalogueVehicleStudio vehicle={vehicle} /></div> : <div className={`relative mx-2 aspect-[16/10] overflow-hidden ${theme}`}>
      ...
      <p className="absolute inset-x-5 bottom-5 text-center text-[9px] tracking-[.18em] text-white/34 uppercase">Official imagery coming soon</p>
    </div>}

    <div className="flex flex-1 flex-col px-6 pb-6 pt-4">
      <p className="text-center text-sm text-white/58">{vehicle.tagline}</p>
      {vehicle.pageReady ? <div className="relative mt-auto min-h-[76px] pt-5"><span aria-hidden="true" className="absolute inset-x-0 top-7 hidden min-h-12 items-center justify-center text-[10px] tracking-[.14em] text-white/32 uppercase transition-opacity lg:flex lg:group-hover:opacity-0 lg:group-focus-within:opacity-0">Explore options</span><div className="grid grid-cols-2 border-y border-white/12 lg:translate-y-2 lg:opacity-0 lg:transition lg:duration-300 lg:group-hover:translate-y-0 lg:group-hover:opacity-100 lg:group-focus-within:translate-y-0 lg:group-focus-within:opacity-100">
