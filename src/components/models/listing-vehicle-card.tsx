...
export function ListingVehicleCard({ vehicle, index }: { vehicle: ModelCatalogueEntry; index: number }) {
  ...
  return <article className="group relative isolate flex w-full min-w-0 flex-col overflow-hidden border border-white/10 bg-[#0d1012] transition-colors duration-300 hover:border-white/28 focus-within:border-white/35">
    ...
    <div className="relative z-10 flex flex-1 flex-col px-6 pb-6 pt-4">
      ...
      {hasStudio ? <div className="relative mt-auto min-h-[76px] pt-5"><span aria-hidden="true" className="absolute inset-x-0 top-7 hidden min-h-12 items-center justify-center text-[10px] tracking-[.14em] text-white/32 uppercase transition-opacity lg:flex lg:group-hover:opacity-0 lg:group-focus-within:opacity-0">Explore options</span><div className="grid grid-cols-2 border-y border-white/12 lg:translate-y-2 lg:opacity-0 lg:transition lg:duration-300 lg:group-hover:translate-y-0 lg:group-hover:opacity-100 lg:group-focus-within:translate-y-0 lg:group-focus-within:opacity-100">
